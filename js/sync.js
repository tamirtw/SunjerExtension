...
// data source is elements.styles here
// elements.options.sync is used to check if sync is enabled / disabled

// loads data from bookmark (if it exists). If no data is returned, saves local data in the bookmark
function sync() {
	console.log("Syncing data from sunjer_data");
    loadSyncData(function(data) {
        if (data) {
            if (data != elements.styles)
                onSync(data);
        }
        else {
...
function onBookmarkUpdate(id, properties) {
    if (elements.options.sync && id == syncId && !saveSyncDataWasCalled) {
        sync();
    }
}

function onBookmarkCreate(id, bookmark) {
    if (elements.options.sync && bookmark.parentId == syncFolderId && !saveSyncDataWasCalled) {
        sync();
    }
}